  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs bg-slate-100 px-3 py-1 rounded-full text-slate-500">{questions.length} ข้อ</span>
      </div>
      <ol className="space-y-4">
        {questions.map((q, idx) => {
          ...
          return (
            <li key={idx} className="rounded-2xl border border-slate-200 bg-white/30 p-5 hover:border-slate-300 transition">
              <div className="mb-4 font-medium flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <span className="leading-snug text-slate-800 text-lg">{idx + 1}. {q.question}</span>
                </div>
              ...
              {q.type === "mcq" ? (
                <div className="grid gap-2 pl-5">
                  {q.choices?.map((c, i) => {
                    ...
                    if (!isThisQuestionSubmitted) cls += selected ? "border-indigo-500 bg-indigo-500/10" : "border-slate-200 hover:bg-slate-100 hover:border-slate-300";
                    else cls += isCorrectChoice ? "border-emerald-500 bg-emerald-500/10" : wrongSelected ? "border-red-500 bg-red-500/10" : "border-slate-200 opacity-60";
                    ...
                    return (
                      <label key={i} className={cls}>
                        ...
                        />
                        <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs mr-2 transition ${selected ? "border-indigo-400 text-indigo-400" : "border-slate-300 text-slate-500"}`}>
                          {letter}
                        </div>
                ...
                <div className="flex gap-3 pl-5">
                  {["true", "false"].map((v) => {
                    ...
                    if (!isThisQuestionSubmitted) cls += selected ? "border-indigo-500 bg-indigo-500/10" : "border-slate-200 hover:bg-slate-100";
                    else cls += isCorrectChoice ? "border-emerald-500 bg-emerald-500/10" : wrongSelected ? "border-red-500 bg-red-500/10" : "border-slate-200 opacity-60";
                    ...
                    return (
                      <label key={v} className={cls}>
                        <input
                          ...
                          onChange={(e) => onAnswerChange(idx, e.target.value)}
                        />
                        <span className={selected ? "font-semibold text-white" : "text-slate-500"}>
              ...
              {isThisQuestionSubmitted && (
                <div className="mt-4 p-4 rounded-xl bg-slate-50 border border-slate-200 text-sm ml-5">
                  <div className="text-emerald-400 font-semibold mb-1">เฉลย: {String(q.answer).toUpperCase()}</div>
                  {q.explain && <div className="text-slate-500">{q.explain}</div>}
                </div>
              )}
            </li>
          );
      ...
      <div className="mt-8 pt-6 border-t border-slate-200 flex items-center justify-between">
        ...
        <PrimaryBtn
          ...
          className={`px-8 bg-gradient-to-r from-blue-600 to-blue-600 hover:from-blue-500 hover:to-blue-500 ${!isAllAnswered && !isOverallSubmitted ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          ตรวจคะแนน
        </PrimaryBtn>
      </div>
      ...
      {isOverallSubmitted && score !== null && (
        <div className="mt-8 text-center p-8 rounded-3xl bg-white/40 border border-slate-200/80 shadow-2xl backdrop-blur-sm animate-in zoom-in-95 duration-500">
          <div className="text-slate-500 text-sm font-bold tracking-widest mb-2">คะแนนของคุณ</div>
          <div className="flex items-baseline justify-center gap-2">
            <span className="text-6xl font-black bg-gradient-to-tr from-emerald-400 to-teal-400 bg-clip-text text-transparent">
              {score}
            </span>
